...
void imprimirMetricas(const Grafo& G, const std::string& nombre) {
    ...
    size_t maxComp = 0;
    for (auto& c : comps)
        maxComp = std::max(maxComp, c.size());
...
int main(int argc, char* argv[]) {
    ...
    std::vector<EstrategiaFn> estrategias = {
        ...
        {"rastreo_bfs", [](const Grafo& g){ return Estrategias::rastreaContacnosBFS(g); }},
